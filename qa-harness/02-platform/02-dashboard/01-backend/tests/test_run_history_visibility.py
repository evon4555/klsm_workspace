...
def test_functional_run_history_excludes_performance_runs(tmp_path, monkeypatch):
    ...
    try:
        ...
        db.add_all([functional, performance])
        db.commit()
        ...
        assert [run["id"] for run in default_history] == [functional_id]
        assert {run["id"] for run in all_history} == {functional_id, performance_id}
