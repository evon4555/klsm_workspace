...
# The 6 numbered stages every date package may have.
STAGE_DEFS = [
...
STAGE_CHECKS = {
    ...
    "06-release":          lambda p: _check_simple_folder(p, "06-release", "Release"),
}
...
def scan_package(pkg_dir: Path) -> PackageResult:
    ...
    # Verdict + next-action heuristic
    statuses = [s.status for s in stages]
    if all(s == "pass" for s in statuses):
        verdict = "complete"
    elif all(s in ("empty",) for s in statuses):
        verdict = "not-started"
    else:
        verdict = "in-progress"
