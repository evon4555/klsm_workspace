...
def list_runs():
    ...
    db = get_db()
    try:
        runs = db.query(TestRun).order_by(TestRun.id.desc()).limit(20).all()
...
async def rerun_one_scenario(scenario_id: int):
    ...
    asyncio.create_task(
        _execute_run(new_id, tags, env, names=[name])
    )
...
def _cases_for_features(features) -> list[str]:
    """Read the selected .feature files and collect their SIT-TC-WEB-AUTH
    case numbers, so update_evidence.py can be run for just those cases."""
    cases: set[str] = set()
    for feat in features or []:
        fpath = Path(PROJECT_ROOT) / feat
        if not fpath.exists() and feat.startswith("features/"):
            fpath = FEATURES_ROOT / feat.removeprefix("features/")
        if not fpath.exists() and "/" not in feat and "\\" not in feat:
            fpath = FEATURES_ROOT / feat
        if fpath.exists():
            cases.update(re.findall(r"SIT-TC-WEB-AUTH-(\d+)",
                                    fpath.read_text(encoding="utf-8")))
    return sorted(cases)


def _run_sync(cases: list[str], env: str):
    """Run tools/update_evidence.py as a subprocess, streaming stdout into
    the in-memory _sync_state log."""
    cmd = [sys.executable, str(TOOLS_ROOT / "update_evidence.py")] + cases
    ...
    try:
        proc = subprocess.Popen(
            cmd, cwd=PROJECT_ROOT, env=proc_env, text=True,
...
def start_sync_evidence(req: SyncRequest):
    ...
    cases = _cases_for_features(req.features)
    ...
    _sync_state.update(running=True, log=[], exit_code=None,
                       ...
                       finished_at=None, scope=scope)
    threading.Thread(target=_run_sync, args=(cases, req.env), daemon=True).start()
    return {"status": "running", "scope": scope, "cases": cases or "all"}
...
def evidence_status():
    ...
    table joins this by case number to show a 'Sync to Excel' column."""
    import json
    f = ARTIFACTS_ROOT / "evidence_sync.json"
    if not f.exists():
        return {"updated_at": None, "cases": {}}
    try:
...
def quality_evidence_status():
    """Evidence package coverage for Quality System deliverables.

    Scans West Kowloon's 03-evidence/**/manifest.json files. Only packages
    whose manifest gate is trusted count as evidence-backed deliverables.
    """
    manifests_root = PROJECT_EVIDENCE_ROOT
    packages = []
    errors = []
    for path in sorted(manifests_root.rglob("manifest.json")):
    ...
    return {
        "evidenceRoot": str(PROJECT_EVIDENCE_ROOT).replace("\\", "/"),
...
# ---------------------------------------------------------------------------
# Case history — unified timeline aggregating ALL evidence sources for one
# test case (Behave runs from the DB, update_evidence.py screenshot batches,
# linked ZenTao bugs, and the current xlsx snapshot).
# Endpoint:  GET /api/history/case/{case_id}
# case_id may be the full "SIT-TC-WEB-AUTH-034" or just the 3-digit "034".
# ---------------------------------------------------------------------------
...
def _normalise_case_id(raw: str) -> tuple[str, str]:
    """('SIT-TC-WEB-AUTH-034', '034') from any of '34' / '034' / full id."""
...
def _evidence_runs_for_case(case_short: str) -> list[dict]:
...
def case_history(case_id: str):
    """Unified timeline for a single test case (Behave runs + evidence
    batches + ZenTao bugs + current xlsx row). Newest first."""
...
def _zt_session():
    ...
    return session
ZENTAO_DEFAULT_PRODUCT_ID = 146     # 西九
...
def _short_scenario_label(name: str) -> str:
    """Strip the SIT-TC-WEB-AUTH-XXX prefix from a scenario name so the
    bug title doesn't restate the whole test id."""
    return re.sub(r"^\s*SIT-TC-WEB-AUTH-\d+\s*", "", name or "").strip()
...
_CASE_ID_RE = re.compile(r"SIT-TC-WEB-AUTH-(\d+)", re.I)


def _latest_screenshots_for_case(case_id: str, limit: int = 4) -> list[Path]:
    """Find the most-recent evidence_* dir for this case id (e.g. '044'),
...
def _bug_defaults_from_scenario(s: TestScenario,
    ...
    # Screenshots: pull latest from artifacts/screenshots/evidence_*/tcXXX/
    cid_m = _CASE_ID_RE.search(s.name or "")
    screenshots_html = "<p><i>(no screenshots found for this case)</i></p>"
    if cid_m:
        shots = _latest_screenshots_for_case(cid_m.group(1))
        if shots:
            parts = []
            for sh in shots:
    ...
    steps = (
        ...
        f"<tr><td><b>标签 Tags</b></td><td>{s.tags or '(none)'}</td></tr>"
...
async def ws_logs(websocket: WebSocket, run_id: int):
    ...
    for line in runner.logs.get(run_id, []):
        await websocket.send_text(line)

    # Subscribe to new lines via an asyncio.Queue
    queue: asyncio.Queue = asyncio.Queue()
    runner.subscribe(run_id, queue)
...
def list_features():
    ...
    for fpath in sorted(features_dir.glob("**/*.feature")):
        ...
        results.append({
            "file": str(fpath.relative_to(Path(PROJECT_ROOT))).replace("\\", "/"),
            "feature": feature_name,
            "scenarios": scenarios,
        })

    return {"features": results, "tags": sorted(all_tags)}
...
def get_trends(limit: int = 30):
    ...
    try:
        ...
        # Overall summary
        all_runs = db.query(TestRun).filter(TestRun.status != "running").all()
...
def get_flaky_tests(limit: int = 15):
    """Detect flaky tests by analyzing status flips across recent runs."""
    db = get_db()
    try:
        runs = (
            db.query(TestRun)
            .filter(TestRun.status != "running", TestRun.status != "error")
            .order_by(TestRun.id.desc())
            .limit(limit)
            .all()
        )
        ...
        return {"flaky_tests": flaky, "runs_analyzed": len(runs)}
    finally:
        db.close()
...
def perf_latest_report():
    """Get the latest performance test report parsed from Locust artifacts."""
    report = _parse_locust_report(_current_perf_artifact_prefix())
    if report:
        return report
...
async def start_perf_test(req: PerfTestRequest):
    ...
    locustfile = Path(PROJECT_ROOT) / req.locustfile
    if not locustfile.exists() and req.locustfile.startswith("tests/"):
        locustfile = TESTS_ROOT / req.locustfile.removeprefix("tests/")
    ...
    for old_csv in PERF_ARTIFACT_PREFIX.parent.glob(f"{PERF_ARTIFACT_PREFIX.name}_*.csv"):
        try:
            old_csv.unlink()
        except OSError:
            pass
...
def _run_api_smoke(env: str) -> None:
    cmd = [str(_API_SMOKE_PYTHON), "-m", "pytest", "02-tests/api/api_smoke", "-q"]
    proc_env = dict(os.environ)
    proc_env["ENV"] = env
    proc_env["PYTHONIOENCODING"] = "utf-8"
    proc_env["PYTHONUTF8"] = "1"
    artifact_mtime_before = _file_mtime_ns(_API_SMOKE_PATH)

    try:
        with _api_smoke_lock:
            _api_smoke_state["log"].append(f"[api-smoke] cwd={PROJECT_ROOT}")
        ...
        artifact_mtime_after = _file_mtime_ns(_API_SMOKE_PATH)
        ...
        if artifact_updated:
            db = get_db()
            try:
                _ingest_latest_api_smoke_json(db)
            finally:
                db.close()
    except Exception as exc:  # noqa: BLE001
        with _api_smoke_lock:
...
def _ingest_latest_api_smoke_json(db) -> None:
    ...
    if not _API_SMOKE_PATH.exists():
        return
    try:
        payload = _json.loads(_API_SMOKE_PATH.read_text(encoding="utf-8"))
    except Exception:
        return
...
async def get_api_monitor_endpoints():
    ...
    if not _API_SMOKE_PATH.exists():
        return {
            ...
            "endpoints": [],
            "_note": "No smoke run on record yet. Run from the project automation root: pytest 02-tests/api/api_smoke -v",
        }
    try:
        return _json.loads(_API_SMOKE_PATH.read_text(encoding="utf-8"))
    except Exception as exc:
        return {
            "project": "",
            "website_url": "",
            ...
            "summary": {"total": 0, "up": 0, "degraded": 0, "down": 0, "skipped": 0},
            "endpoints": [],
            "_error": f"failed to read {_API_SMOKE_PATH.name}: {exc}",
        }


@app.get("/api/api-monitor/trends")
async def get_api_monitor_trends(hours: int = 24, endpoint_name: str | None = None):
    ...
    try:
        ...
        runs = (
            db.query(ApiMonitorRun)
            .filter(ApiMonitorRun.ran_at >= cutoff)
...
async def get_api_monitor_layers():
    """Per-layer (api_smoke / api_contract / api_functional / api_ui_mixed) test result counts
    from the latest pytest run of project 02-automation/02-tests/api."""
    import json as _json
    if not _API_LAYER_PATH.exists():
        return {
            "ran_at": None,
            "layers": [],
    ...
    try:
        return _json.loads(_API_LAYER_PATH.read_text(encoding="utf-8"))
    except Exception as exc:
        return {"ran_at": None, "layers": [], "_error": str(exc)}
...
async def _execute_run(
    ...
    exit_code, scenarios = await runner.run_behave(
...
def _scenario_dict(s: TestScenario) -> dict:
...
def _current_perf_artifact_prefix() -> Path:
...
_ZT_QA_TASK_MATRIX_CACHE: Optional[tuple[float, dict]] = None
...
def _fetch_qa_task_matrix(refresh: bool = False) -> dict:
    global _ZT_QA_TASK_MATRIX_CACHE
    now = _time.time()
    if not refresh and _ZT_QA_TASK_MATRIX_CACHE and (now - _ZT_QA_TASK_MATRIX_CACHE[0]) < _ZT_QA_TASK_MATRIX_TTL:
        payload = dict(_ZT_QA_TASK_MATRIX_CACHE[1])
        payload["cached"] = True
        return payload
    ...
    _ZT_QA_TASK_MATRIX_CACHE = (now, payload)
    return payload


@app.get("/api/zentao/qa-task-matrix")
def zentao_qa_task_matrix(refresh: bool = False):
    """Global QA task count matrix across all open and closed executions."""
    return _fetch_qa_task_matrix(refresh=refresh)
...
def zentao_dashboard(
    ...
    Cached 5 minutes per product_id — pass
    refresh=true to bypass."""
    cache_key = f"product:{product_id}:max:{max_iterations}"
    now = _time.time()
    ...
    # --- 1. Executions list — fetch BOTH "doing" and "wait" so upcoming work
    # is also reachable. `/products/{pid}/executions` 404s on this SaaS;
    # we list at the top level and ignore product filter for executions
    # (execution.product is unreliable on chandao SaaS).
    from concurrent.futures import ThreadPoolExecutor
    ...
    if product_id > 0:
        ...
        # No product selected — reuse the testcases we already fetched per
        # execution in the parallel block. Sum them up, dedupe by case id.
        seen = set()
        for ex in active_execs:
...
#
# Reads the live filesystem state of west-kowloon requirement packages and
# returns per-stage health. Filesystem IS the data source — no DB tables.
# Scoped to project=west-kowloon for now (Phase 1 decision).
# ---------------------------------------------------------------------------
...
def get_package_executions(package_id: str):
    """Pull dashboard.db test_scenarios whose tag or name matches any case ID
