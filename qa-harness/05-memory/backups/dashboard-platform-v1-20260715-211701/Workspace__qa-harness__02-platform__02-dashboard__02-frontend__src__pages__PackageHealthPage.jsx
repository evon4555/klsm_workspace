...
export default function PackageHealthPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null)
    try {
      const json = await fetchQualityPackages('west-kowloon')
      setData(json)
      // re-fetch detail if a row was previously selected
      if (selected) {
        const updated = json.packages.find(p => p.id === selected)
        if (updated) {
          setDetail(updated)
        }
      }
  ...
  // -----------------------------------------------------------------------
  // Build table columns: 1 row per package, 6 stage columns + verdict + next
  // -----------------------------------------------------------------------
