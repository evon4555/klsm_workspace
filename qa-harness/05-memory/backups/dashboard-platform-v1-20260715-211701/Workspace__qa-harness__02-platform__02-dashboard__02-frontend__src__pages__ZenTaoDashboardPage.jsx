...
export default function ZenTaoDashboardPage() {
  // Data + fetch state
  ...
  const [productId, setProductId] = useState(146)   // 146 = 西九, set to 0 for "all"
  ...
  const fetchData = useCallback(async (refresh = false) => {
    ...
    setError(null)
    try {
      const url = `/api/zentao/dashboard?product_id=${productId}&max_iterations=${MAX_FETCH}${refresh ? '&refresh=true' : ''}`
  ...
  const fetchQaTaskMatrix = useCallback(async (refresh = false) => {
    ...
    setQaTaskError(null)
    try {
      const resp = await fetch(`/api/zentao/qa-task-matrix${refresh ? '?refresh=true' : ''}`)
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`)
      const json = await resp.json()
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const resp = await fetch('/api/zentao/products')
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`)
      const json = await resp.json()
  ...
  const generateStoryTestcases = useCallback(async (story) => {
    ...
    try {
      const resp = await fetch(`/api/zentao/story/${storyId}/testcases/generate`, {
        method: 'POST',
  ...
  const reqColumns = [
    ...
    { title: '生成用例', dataIndex: 'generate', width: 112,
      render: (_, r) => (
        <Tooltip title="按当前 Story 生成西九标准 xlsx 测试用例">
          <Button
            size="small"
  ...
  return (
    <div style={{ maxWidth: 1400 }}>
        ...
        <Space wrap align="start" size={8} style={{ justifyContent: 'flex-end' }}>
          ...
          <Space direction="vertical" size={2}>
            ...
            <Select
              ...
              showSearch
              value={productId}
              onChange={(v) => setProductId(v ?? 0)}
              style={{ width: 260 }}
              optionFilterProp="label"
              options={[
                { value: 0, label: '全部产品 (0)' },
                ...products.map((p) => ({
                  value: p.id,
                  label: `${p.name} (${p.id})${p.status && !['normal', 'unknown'].includes(p.status) ? ` - ${p.status}` : ''}`,
                })),
              ]}
            />
          </Space>
        ...
        <Col xs={12} sm={8} md={5}>
          <Card size="small" style={{ borderRadius: 10, border: '1px solid #f0f0f0', boxShadow: 'none' }}>
            <Statistic
              ...
              valueStyle={{ fontSize: 22 }}
            />
            <Text type="secondary" style={{ fontSize: 11 }}>全部执行</Text>
          </Card>
        </Col>
        <Col xs={12} sm={8} md={5}>
          <Card size="small" style={{ borderRadius: 10, border: '1px solid #f0f0f0', boxShadow: 'none' }}>
      ...
      <SectionHeader
        ...
        icon={<RobotOutlined />}
        title="全量自动化（回归）"
        subtitle="公司级整体进度 · 不随上方执行切换 · 反映回归测试整体水平"
      ...
      <SectionHeader
        ...
        title="任务数量分布"
        subtitle="全局 QA · 仅测试类任务（type=test 或名称含【测试】）· 包含已关闭"
        extra={<Tag color="blue" style={{ marginRight: 0, fontSize: 11 }}>全部执行</Tag>}
      />
