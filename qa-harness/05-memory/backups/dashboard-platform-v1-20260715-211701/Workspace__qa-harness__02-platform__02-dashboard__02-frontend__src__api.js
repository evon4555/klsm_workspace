 ...
 */
export async function startRun(features, env) {
  const res = await fetch('/api/runs', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ features: features && features.length ? features : null, env }),
  })
  return res.json()
}
 ...
 */
export async function startSyncEvidence(features, env = 'sit') {
...
export async function getEvidenceStatus() {
  const res = await fetch('/api/evidence-status')
  return res.json()
}
 ...
 * ZenTao bugs, and the current xlsx snapshot. caseId may be the full
 * "SIT-TC-WEB-AUTH-034" or just "034".
 */
...
export async function fetchTrends(limit = 30) {
  const res = await fetch(`/api/stats/trends?limit=${limit}`)
  return res.json()
}
...
export async function fetchFlakyTests(limit = 15) {
  const res = await fetch(`/api/stats/flaky?limit=${limit}`)
  return res.json()
}

/** Get the latest API smoke run snapshot for the API Monitor page. */
export async function fetchApiMonitorEndpoints() {
  const res = await fetch('/api/api-monitor/endpoints')
  return res.json()
}

/** Get the per-layer (smoke / contract / hybrid / functional) test counts. */
export async function fetchApiMonitorLayers() {
  const res = await fetch('/api/api-monitor/layers')
  return res.json()
}

/** Trigger the API smoke pytest layer in the dashboard backend. */
export async function startApiMonitorSmokeRun(env = 'sit') {
...
export async function fetchApiMonitorSmokeRun() {
  const res = await fetch('/api/api-monitor/run-smoke')
...
export async function fetchApiMonitorTrends(hours = 168, endpointName = null) {
 ...
 * List every west-kowloon requirement package with per-stage status.
