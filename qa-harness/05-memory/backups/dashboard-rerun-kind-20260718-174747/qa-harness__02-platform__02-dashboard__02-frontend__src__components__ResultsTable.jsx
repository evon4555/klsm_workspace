...
import { openBugForScenario, rerunScenario, getCaseHistory } from '../api.js'
...
function formatTs(iso) {
  ...
  // Trim microseconds, keep 'YYYY-MM-DD HH:MM:SS'
  return iso.replace('T', ' ').replace(/\.\d+$/, '').slice(0, 19)
}

/**
 * Build the table columns. The "Sync to Excel" column (right after Duration)
 ...
 *
 * `rerunOne(scenario)` triggers a single-scenario rerun and shows a toast
 * with a link to the new run id.
 */
function buildColumns(evidenceMap, openBugFor, rerunOne, rerunningId,
  ...
  return [
    ...
    },
    {
      ...
      render: (s) => (
        <Tag color={statusColor[s] || 'default'} style={{ borderRadius: 4, fontWeight: 500 }}>
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const rerunBtn = (
          <Tooltip title="Rerun just this scenario (spawns a new run)">
            <Button
              size="small"
              icon={<ReloadOutlined />}
...
export default function ResultsTable({
  ...
  const rerunOne = async (scenario) => {
    ...
    try {
      const r = await rerunScenario(scenario.id)
      if (r.error) {
        ...
        messageApi.success({
          content: (
            <>
              Started run <strong>#{r.id}</strong> for{' '}
              <code>{scenario.name.slice(0, 30)}…</code>
            </>
          ),
          duration: 6,
        })
          ...
          <Space>
            ...
            {historyModal.data?.counts && (
              <span style={{ fontSize: 12, color: '#888' }}>
                ({historyModal.data.counts.execution_results ?? historyModal.data.counts.behave_runs} execution results)
              </span>
            )}
          </Space>
        }
