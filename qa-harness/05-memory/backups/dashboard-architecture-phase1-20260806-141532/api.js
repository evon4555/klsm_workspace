...
export async function listFeatures(project = 'west-kowloon', packageId = null) {
  ...
  const res = await fetch(`/api/features${query ? `?${query}` : ''}`)
  return res.json()
}
...
export async function promotePackageExecution(packageId, runId) {
  const path = packageId.split('/').map(encodeURIComponent).join('/')
  const res = await fetch(`/api/quality-system/packages/${path}/executions/${runId}/promote`, {
    method: 'POST',
  })
  if (!res.ok) throw new Error(`Promote execution snapshot: HTTP ${res.status}`)
  return res.json()
}
