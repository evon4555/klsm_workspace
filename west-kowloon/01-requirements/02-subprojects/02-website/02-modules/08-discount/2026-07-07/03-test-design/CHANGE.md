...
## v4 - 2026-07-07 (current)

**Source dependency**:
- Test Manager review comments captured in `../04-test-case-review/test-case-review-promo-code-bank-card-offer.docx`, Final Decision Q-3.

**Trigger**:
- Human review comments:
  - Translate test cases to English.
  - Remove Markdown bold markers from Test Steps so steps start as plain `1.`.
  - Leave Test Data blank when no specific test data is required.

**Changes**:
- Translated all 20 test cases to English in `test-cases-promo-code-bank-card-offer.md`.
- Removed `**...**` Markdown bold markers from Test Steps.
- Cleared Test Data for BCO-001 and BCO-016 because the required setup is already stated in Preconditions and no separate data value is needed.
- Regenerated `test-cases-promo-code-bank-card-offer.xlsx` from the approved project template.
- Case IDs, case count, priorities, severities, scope, and old `2026-06-25` package isolation remain unchanged.

**Previous-version snapshot**:
- `.iterations/v3/`

**Review status**:
- Awaiting Test Manager re-review and sign-off after v4 updates.
...
- Superseded by v4 after Test Manager review comments.
