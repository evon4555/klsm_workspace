...
def _render_api_evidence_png(evidence: dict[str, Any], path: Path) -> None:
    ...
    before_requests = evidence.get("before_requests") or []
    height = max(
        960,
        670
        + len(observations) * 74
        + len(setup_requests) * 52
        + len(before_requests) * 52
        + len(readback_requests) * 52,
    )
    width = 1500
    margin = 42
    ...
    y = margin
    draw.text((margin, y), str(evidence.get("title") or "API Readback Evidence"), font=title_font, fill="#111827")
    ...
    if before_requests:
        y = _draw_section_title(draw, "Before Readback API", margin, y, h_font)
        for idx, request in enumerate(before_requests, start=1):
            line = (
                f"{idx}. {request.get('method', '')} {request.get('path', '')} "
...
def _draw_observation_table(
    ...
    for item in observations:
        ...
        expect_change = item.get("expect_change")
        if expect_change is False:
            row_passed = bool(item.get("passed")) and changed is False
        else:
            row_passed = (
