...
Current fixed execution source: dashboard run `200`, `13 passed / 0 failed /
0 errored / 0 skipped`, with per-case UI/Mixed screenshots and API transition
evidence registered in the runtime screenshot manifest. Cases
`SIT-TC-STD-CONFIG-002` and `SIT-TC-STD-CONFIG-015` are removed from current
scope and retained only as audit history.
