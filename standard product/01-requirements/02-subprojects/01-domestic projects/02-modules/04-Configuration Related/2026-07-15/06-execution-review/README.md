...
Current status: execution review drafted. The current 13-case scope passed in
dashboard run `200` with UI/Mixed screenshots and API transition evidence.
`SIT-TC-STD-CONFIG-015` was removed from current scope by Test Manager decision
on 2026-07-17, and `SIT-TC-STD-CONFIG-002` was removed on 2026-07-18, so no
manual execution case remains.
...
Run `194` is superseded and must not be used as the current execution-review
source because it still included removed case `SIT-TC-STD-CONFIG-002`.
