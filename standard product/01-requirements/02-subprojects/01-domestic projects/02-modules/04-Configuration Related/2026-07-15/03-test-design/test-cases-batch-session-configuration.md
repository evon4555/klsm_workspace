...
| Current Status | Test Manager signed off; post sign-off scope adjustments removed `SIT-TC-STD-CONFIG-015` and data-dependent no-selection case `SIT-TC-STD-CONFIG-002` |
...
| 2026-07-17 | Test Manager / QA1 (AI) | Removed `SIT-TC-STD-CONFIG-015` because the basic save-failure prompt case is not necessary for this package. Regenerated the workbook with 14 current-scope cases. | Completed |
| 2026-07-18 | Test Manager / QA1 (AI) | Removed `SIT-TC-STD-CONFIG-002` because the no-selection scenario depends on unstable list/filter data and current evidence was not reliable. Regenerated the workbook with 13 current-scope cases. | Completed |
...
| Q-4 | superseded on 2026-07-17 | Basic save-failure prompt coverage is removed from the current package scope; full partial-success or rollback matrix remains out of scope. |
...
- Treat preview row deletion, cancel / close behavior, and mandatory-field validation as shared component behavior instead of repeating them by every session type and action.
...
| No-selection batch-operation guard | Removed from current scope | Test Manager confirmed on 2026-07-18 that `SIT-TC-STD-CONFIG-002` should be removed because the current evidence depends on unstable list/filter data. |
| Basic failure prompt | Removed from scope | Test Manager confirmed on 2026-07-17 that `SIT-TC-STD-CONFIG-015` is unnecessary for this package. |
...
|----|----|----|----|
| None | No open test-design review point after the 2026-07-18 scope adjustment. | N/A | Proceed with the 13 current-scope cases. |
