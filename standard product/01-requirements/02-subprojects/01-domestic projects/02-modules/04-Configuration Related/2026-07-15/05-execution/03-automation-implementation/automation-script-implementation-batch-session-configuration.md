...
| Latest Dashboard Run | `200` |
| Latest Result | `13 passed / 0 failed / 0 errored / 0 skipped` |
...
The signed automation assessment and post sign-off scope adjustments leave 13
current-scope cases, all selected as automatable:

- `SIT-TC-STD-CONFIG-001`
- `SIT-TC-STD-CONFIG-003` to `SIT-TC-STD-CONFIG-014`
...
`SIT-TC-STD-CONFIG-002` was removed from current scope by Test Manager decision
on 2026-07-18 because the no-selection scenario depends on unstable list/filter
data and current evidence is not reliable.
...
| UI | `SIT-TC-STD-CONFIG-001`, `012`, `014` | 3 | Behave steps drive the real Standard Product admin UI with Playwright. |
...
| Runtime case evidence | `D:\Workspace\standard product\02-automation\07-artifacts\batch_session_configuration\run-200\...\*.png`; API cases also write paired `*.json` evidence |
...
```text
GET http://127.0.0.1:8002/api/runs/200
run id: 200
...
- Mixed case `SIT-TC-STD-CONFIG-013` records API setup/action/readback as one summary evidence file and uses Playwright for the final user-facing UI assertion screenshot.
